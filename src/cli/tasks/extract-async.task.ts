...
import { hideCursor, showCursor } from '../../utils/logger-helpers.js';
...
export class ExtractAsyncTask {
	...
	protected async extract() {
		...
		const files = (await Promise.all(this.inputs.map((pattern) => this.getFiles(pattern)))).flat();
		...
		hideCursor();
		const extracted = await parseFiles(allFileContents);
		showCursor();
		collectionTypes.push(...extracted);
